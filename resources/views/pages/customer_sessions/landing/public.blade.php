    ...
    <div class="ao-wrap">
        ...
        <section class="ao-public-hero mb-5">
            ...
            <div class="ao-public-hero-copy">
                <span class="ao-public-kicker">ACES LACROSSE TRAINING</span>
                <h1>Train Hard.<br>Play Harder.</h1>
                <p>Browse upcoming ACES Training sessions and packages without signing in. Login is required only when you reserve a player or continue through checkout.</p>
        ...
        <section id="trainingSessions" class="mb-5">
            ...
            <div class="ao-grid-3">
                @forelse($sessions as $session)
                    ...
                    <div class="modal fade" id="sessionDetails{{ $session->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-lg modal-dialog-centered"><div class="modal-content ao-modal-card">
                            ...
                            <div class="modal-body p-4">
                                <p>{{ $session->description ?: 'Focused ACES Lacrosse training session.' }}</p>
                                <div class="ao-session-meta">
                    ...
                    @endif
                @empty
                    <div class="ao-panel p-5 text-center" style="grid-column:1/-1"><h3>No upcoming sessions yet</h3><p class="ao-muted mb-0">Check back soon for new ACES Lacrosse Training dates.</p></div>
                @endforelse
            </div>
        </section>
            ...
            <div class="ao-grid-3">
                @forelse($featuredPackages as $package)
                    ...
                    <article class="ao-panel p-4 d-flex flex-column">
                        ...
                        <div class="mb-3"><small class="ao-muted">{{ $guestTier ? 'Guest Price' : 'Member Price' }}</small><div style="font-size:34px;font-weight:900;color:#611eb2">${{ number_format($displayPrice, 2) }}</div></div>
...
<style>
.ao-public-hero{position:relative;overflow:hidden;min-height:520px;border-radius:24px;background:url('{{ asset('public/assets/images/hero-bg.jpg') }}') center/cover no-repeat;box-shadow:0 24px 55px rgba(37,20,55,.16);display:flex;align-items:center;padding:64px}.ao-public-hero-overlay{position:absolute;inset:0;background:linear-gradient(90deg,rgba(23,16,33,.96),rgba(23,16,33,.72) 52%,rgba(23,16,33,.18))}.ao-public-hero-copy{position:relative;z-index:2;max-width:650px;color:#fff}.ao-public-kicker{font-size:12px;font-weight:900;letter-spacing:.18em;color:#c897ff}.ao-public-hero h1{font-size:clamp(50px,6vw,80px);line-height:.93;font-weight:900;letter-spacing:-.05em;margin:15px 0 20px;color:#fff}.ao-public-hero p{max-width:580px;color:#e5ddee;font-size:18px;line-height:1.6;margin-bottom:28px}.ao-public-outline,.ao-public-ghost{border:1px solid rgba(255,255,255,.7);background:rgba(255,255,255,.06);color:#fff}.ao-public-outline:hover,.ao-public-ghost:hover{background:rgba(255,255,255,.16);color:#fff}.ao-player-option{display:flex;align-items:center;width:100%;padding:13px 14px;margin-bottom:8px;border:1px solid #e3e8ef;border-radius:12px;cursor:pointer}.ao-player-option:hover{border-color:#611eb2;background:#faf7ff}@media(max-width:767px){.ao-public-hero{min-height:500px;padding:36px 26px}.ao-public-hero p{font-size:16px}.ao-public-hero h1{font-size:49px}}
</style>
@endsection
