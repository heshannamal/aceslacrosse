...
@section('title', ($mode ?? 'login') === 'set_password' ? 'Create Training Password | ACES Lacrosse' : 'Training Login | ACES Lacrosse')
        ...
        <div class="training-auth-card">
            <section class="training-auth-visual">
                ...
                <div class="training-auth-visual-content">
                    <img src="{{ asset('public/assets/images/ACES-logo.webp') }}" alt="ACES Lacrosse" class="training-auth-brand">
                    ...
                    <div class="training-auth-visual-copy">
                        ...
                        <a href="{{ route('em.customer.index') }}#trainingSessions" class="training-auth-browse-btn">
                            Browse Sessions
            ...
            <section class="training-auth-form-panel">
                <img src="{{ asset('public/assets/images/ACES-logo.webp') }}" alt="ACES Lacrosse" class="training-auth-form-logo">

                @if($isSetPassword)
                    <div class="training-kicker">First time here?</div>
                    <h1>Create Password</h1>
                    <p class="training-auth-intro">
                        Your parent account already exists in ACES Lacrosse Training. Create your password to continue booking sessions.
                    </p>
...
<style>
.training-auth-page{--outlaws-red:#611eb2;--outlaws-dark:#171021;--outlaws-muted:#667085;min-height:100vh;padding:54px 24px 74px;background:radial-gradient(circle at 7% 8%,rgba(97,30,178,.08),transparent 28%),linear-gradient(180deg,#fff 0%,#fbfbfc 100%);font-family:'Roboto',Arial,sans-serif}.training-auth-shell{max-width:1160px;margin:0 auto}.training-auth-alert{margin-bottom:16px;padding:13px 18px;border-radius:12px;font-weight:700;display:flex;gap:8px;align-items:center}.training-auth-alert.success{color:#166534;background:#ecfdf3;border:1px solid #bbf7d0}.training-auth-alert.error{color:#991b1b;background:#fff1f2;border:1px solid #fecdd3}.training-auth-alert.info{color:#1d4ed8;background:#eff6ff;border:1px solid #bfdbfe}.training-auth-card{display:grid;grid-template-columns:1fr 1fr;overflow:hidden;background:#fff;border-radius:28px;box-shadow:0 26px 70px rgba(38,20,54,.14);border:1px solid #eaecf0}.training-auth-visual{position:relative;min-height:650px;background:url('{{ asset('public/assets/images/hero-bg.jpg') }}') center/cover no-repeat}.training-auth-visual-overlay{position:absolute;inset:0;background:linear-gradient(180deg,rgba(23,16,33,.28),rgba(23,16,33,.58) 48%,rgba(12,7,17,.94))}.training-auth-visual-content{position:relative;z-index:2;height:100%;min-height:inherit;padding:46px;display:flex;flex-direction:column;justify-content:space-between}.training-auth-brand{width:205px;max-height:96px;object-fit:contain;object-position:left center;filter:drop-shadow(0 4px 10px rgba(0,0,0,.25))}.training-auth-pill{display:inline-flex;align-items:center;width:max-content;padding:9px 16px;border:1px solid rgba(200,151,255,.9);border-radius:999px;background:rgba(23,16,33,.52);color:#fff;font-size:12px;line-height:1;font-weight:900;letter-spacing:.18em}.training-auth-visual h2{margin:20px 0 26px;color:#fff;font-family:'Roboto',Arial,sans-serif;font-size:clamp(48px,5.2vw,74px);font-weight:900;line-height:.92;letter-spacing:-.06em}.training-auth-browse-btn{display:inline-flex;align-items:center;gap:11px;padding:14px 22px;border-radius:999px;background:var(--outlaws-red);color:#fff!important;font-weight:900;text-decoration:none;box-shadow:0 14px 26px rgba(97,30,178,.27);transition:.2s ease}.training-auth-browse-btn:hover{transform:translateY(-2px);background:#4d168f}.training-auth-form-panel{padding:48px 52px 38px;background:#fff;display:flex;flex-direction:column;justify-content:center}.training-auth-form-logo{width:205px;max-height:86px;object-fit:contain;object-position:left center;margin-bottom:24px}.training-kicker{color:var(--outlaws-red);font-size:27px;font-weight:900;line-height:1;margin-bottom:8px}.training-auth-form-panel h1{margin:0;color:var(--outlaws-dark);font-family:'Roboto',Arial,sans-serif;font-size:clamp(40px,4vw,58px);font-weight:900;line-height:.98;letter-spacing:-.055em}.training-auth-intro{margin:14px 0 26px;color:var(--outlaws-muted);font-size:15px;font-weight:600;line-height:1.55}.training-field{margin-bottom:18px}.training-field label{display:block;margin-bottom:7px;color:#475467;font-size:13px;font-weight:800}.training-field label span{color:var(--outlaws-red)}.training-input-wrap{min-height:50px;padding:0 14px;border:1px solid #d9dee7;border-radius:13px;background:#fff;display:flex;align-items:center;gap:11px;transition:border-color .2s ease,box-shadow .2s ease}.training-input-wrap:focus-within{border-color:var(--outlaws-red);box-shadow:0 0 0 3px rgba(97,30,178,.1)}.training-input-wrap>i{width:17px;color:#8b95a5;text-align:center}.training-input-wrap input{flex:1;min-width:0;height:48px;border:0;outline:0;background:transparent;color:#111827;font-size:14px;font-weight:700}.training-input-wrap input::placeholder{color:#98a2b3;font-weight:500}.training-password-toggle{border:0;background:transparent;color:#8b95a5;padding:5px}.training-readonly-email{min-height:50px;padding:0 14px;border:1px solid #e4e7ec;border-radius:13px;background:#f8fafc;display:flex;align-items:center;gap:11px;color:#111827;font-weight:800;word-break:break-word}.training-readonly-email i{color:#8b95a5}.training-login-options{display:flex;justify-content:space-between;align-items:center;gap:16px;margin:4px 0 28px;font-size:13px;font-weight:800}.training-login-options a,.training-auth-switch a{color:var(--outlaws-red);text-decoration:none;font-weight:900}.training-remember{display:inline-flex;align-items:center;gap:7px;color:#667085;cursor:pointer}.training-remember input{accent-color:var(--outlaws-red)}.training-login-actions{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding-top:8px}.training-auth-back-btn,.training-auth-submit-btn{min-height:50px;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;gap:10px;text-decoration:none;font-size:15px;font-weight:900;transition:.2s ease}.training-auth-back-btn{border:1.5px solid #171021;background:#fff;color:#171021!important}.training-auth-submit-btn{border:0;background:var(--outlaws-red);color:#fff;box-shadow:0 12px 24px rgba(97,30,178,.22)}.training-auth-back-btn:hover{background:#171021;color:#fff!important}.training-auth-submit-btn:hover{background:#4d168f;transform:translateY(-1px)}.training-auth-switch{margin:22px 0 0;text-align:center;color:#667085;font-size:13px;font-weight:700}@media(max-width:991px){.training-auth-page{padding:28px 16px 54px}.training-auth-card{grid-template-columns:1fr}.training-auth-visual{min-height:410px}.training-auth-visual-content{padding:30px}.training-auth-visual h2{font-size:54px}.training-auth-form-panel{padding:34px 26px 30px}}@media(max-width:720px){.training-login-actions{grid-template-columns:1fr}.training-login-options{align-items:flex-start;flex-direction:column}.training-auth-form-logo{width:180px}.training-auth-form-panel h1{font-size:40px}}@media(max-width:480px){.training-auth-page{padding-inline:10px}.training-auth-card{border-radius:20px}.training-auth-visual-content{padding:24px}.training-auth-visual h2{font-size:44px}.training-auth-form-panel{padding:28px 20px}}
</style>
